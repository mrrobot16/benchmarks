...
Bun.serve({
  ...
  fetch(req) {
    ...
    const height = parsePositiveInt(url.searchParams.get("height"), DEFAULT_HEIGHT);
    const maxIter = parsePositiveInt(url.searchParams.get("maxIter"), DEFAULT_MAX_ITER);
...
console.log(`Bun benchmark server listening on http://127.0.0.1:${PORT}`);
